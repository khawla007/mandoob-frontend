...
create policy service_cases_pro_read on public.service_cases for select
  using (
    tenant_id = ((auth.jwt() -> 'app_metadata' ->> 'tenant_id'))::uuid
    and (auth.jwt() -> 'app_metadata' ->> 'mandoob_role') = 'pro'
  );

revoke insert, update, delete on table public.service_cases from public, anon, authenticated;
