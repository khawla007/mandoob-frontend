-- Supersede cancellation recovery with a fenced, durable delete-intent protocol.
begin;
...
alter table public.pro_credential_evidence_removals
  ...
  add constraint pro_credential_evidence_removals_status_check
    check (status in ('prepared', 'recovering', 'complete', 'cancelled')),
  add constraint pro_credential_evidence_removals_shape check (
    ...
    or
    (status = 'recovering' and storage_path is not null and sanitized_result is null
      and completed_at is null and cancelled_at is null and recovery_actor_id is not null
      and recovery_operation_id is not null)
    or
    (status = 'complete' and storage_path is null and sanitized_result is not null
      and completed_at is not null and cancelled_at is null and recovery_operation_id is null)
    or
...
drop index public.pro_credential_evidence_removals_one_prepared_credential;
create unique index pro_credential_evidence_removals_one_active_credential
  on public.pro_credential_evidence_removals(credential_id)
  where status in ('prepared', 'recovering');
...
begin
  ...
  where removal.credential_id = old.id and removal.status in ('prepared', 'recovering');
  if v_reservation_id is not null
     and current_setting('app.pro_evidence_finalize', true) is distinct from v_reservation_id::text
     and current_setting('app.pro_evidence_recovery_finalize', true) is distinct from v_reservation_id::text then
...
begin
  if not exists (
    ...
    where removal.evidence_id = old.id and removal.status in ('prepared', 'recovering')
      and (
        current_setting('app.pro_evidence_finalize', true) = removal.id::text
        or current_setting('app.pro_evidence_recovery_finalize', true) = removal.id::text
      )
  ) then
...
begin
  ...
  if found then
    ...
    if v_removal.status = 'recovering' then
      raise exception using errcode = 'P0001', message = 'EVIDENCE_REMOVAL_IN_PROGRESS';
    end if;
    if v_removal.status = 'cancelled' then
      ...
      if exists (
        select 1 from public.pro_credential_evidence_removals competing
        where competing.credential_id = v_removal.credential_id
          and competing.id <> v_removal.id
          and competing.status in ('prepared', 'recovering')
      ) then
        raise exception using errcode = 'P0001', message = 'EVIDENCE_REMOVAL_IN_PROGRESS';
      end if;
      ...
      set status = 'prepared', storage_path = v_evidence.storage_path,
          ...
          cancelled_at = null, recovery_actor_id = null, recovery_operation_id = null
  ...
  select pro_profile_id into v_pro_profile_id from public.pro_credentials where id = p_credential_id;
  if not found then raise exception using errcode = 'P0001', message = 'NOT_FOUND'; end if;
  perform public.assert_pro_lifecycle_actor(p_actor_id, v_pro_profile_id, false);
  select * into strict v_credential from public.pro_credentials where id = p_credential_id for update;
  v_replay := public.pro_lifecycle_replay_result('credential', p_credential_id, p_operation_id, p_payload_hash);
...
begin
  ...
  delete from public.pro_credential_evidence where id = p_evidence_id and credential_id = p_credential_id;
  if not found then raise exception using errcode = 'P0001', message = 'NOT_FOUND'; end if;
  update public.pro_credentials set version = version + 1 where id = p_credential_id;
  perform public.write_pro_lifecycle_audit(p_actor_id, v_removal.pro_profile_id,
    'credential_evidence_removed', p_credential_id, v_credential.version + 1);
  v_result := public.pro_credential_masked_result(p_credential_id);
  perform public.store_pro_lifecycle_receipt('credential', p_credential_id,
    p_operation_id, p_payload_hash, v_result);
  update public.pro_credential_evidence_removals
...
declare
  ...
  v_removal public.pro_credential_evidence_removals%rowtype;
begin
  if p_recovery_operation_id is null then
    raise exception using errcode = 'P0001', message = 'INVALID_OPERATION';
  end if;
  ...
  select * into v_removal from public.pro_credential_evidence_removals
  where evidence_id = p_evidence_id and credential_id = p_credential_id
    and pro_profile_id = p_pro_profile_id for update;
  if not found then raise exception using errcode = 'P0001', message = 'NOT_FOUND'; end if;
  if v_removal.status = 'complete' then
    return pg_catalog.jsonb_build_object('status', 'complete', 'credential', v_removal.sanitized_result);
  end if;
  if v_removal.status not in ('prepared', 'recovering', 'cancelled') then
    raise exception using errcode = 'P0001', message = 'RECOVERY_STATE_CHANGED';
  end if;
  if v_removal.status in ('prepared', 'recovering')
     and v_removal.lease_expires_at > pg_catalog.now() then
  ...
  if exists (
    select 1 from public.pro_credential_evidence_removals competing
    where competing.credential_id = v_removal.credential_id and competing.id <> v_removal.id
      and competing.status in ('prepared', 'recovering')
  ) then
    raise exception using errcode = 'P0001', message = 'EVIDENCE_REMOVAL_IN_PROGRESS';
  end if;
  ...
  where id = p_evidence_id and credential_id = p_credential_id
    and pro_profile_id = p_pro_profile_id for update;
  if not found then raise exception using errcode = 'P0001', message = 'RECOVERY_STATE_CHANGED'; end if;
  perform pg_catalog.set_config('app.pro_evidence_recovery_claim', v_removal.id::text, true);
  update public.pro_credential_evidence_removals
  set status = 'recovering', storage_path = v_evidence.storage_path,
      recovery_actor_id = p_actor_id, recovery_operation_id = p_recovery_operation_id,
      lease_expires_at = pg_catalog.now() + interval '5 minutes', cancelled_at = null
  where id = v_removal.id returning * into v_removal;
  return pg_catalog.jsonb_build_object(
    'status', 'recovering', 'recoveryOperationId', v_removal.recovery_operation_id,
    'credentialId', v_removal.credential_id, 'evidenceId', v_removal.evidence_id,
    'storagePath', v_removal.storage_path
  );
end;
$$;
...
begin
  ...
  if not found then raise exception using errcode = 'P0001', message = 'NOT_FOUND'; end if;
  if v_removal.status = 'complete' then return v_removal.sanitized_result; end if;
  if v_removal.status <> 'recovering'
     or v_removal.recovery_actor_id <> p_actor_id
     or v_removal.recovery_operation_id <> p_recovery_operation_id then
    raise exception using errcode = 'P0001', message = 'EVIDENCE_REMOVAL_CLAIM_LOST';
  end if;
  select * into v_credential from public.pro_credentials
  where id = p_credential_id and pro_profile_id = p_pro_profile_id for update;
  if not found or v_credential.version <> v_removal.expected_version
  ...
  perform pg_catalog.set_config('app.pro_evidence_recovery_finalize', v_removal.id::text, true);
  delete from public.pro_credential_evidence
  where id = p_evidence_id and credential_id = p_credential_id and pro_profile_id = p_pro_profile_id;
  if not found then raise exception using errcode = 'P0001', message = 'RECOVERY_STATE_CHANGED'; end if;
  update public.pro_credentials set version = version + 1 where id = p_credential_id;
  insert into public.auth_events (actor_user_id, tenant_id, kind, details)
  values (
    p_actor_id, null, 'pro_lifecycle_changed',
    pg_catalog.jsonb_build_object(
      'action', 'credential_evidence_removal_recovered',
      'targetProProfileId', p_pro_profile_id, 'entityId', p_credential_id,
      'version', v_credential.version + 1,
      'originalActorId', v_removal.actor_id, 'recoveryActorId', p_actor_id
    )
  );
  v_result := public.pro_credential_masked_result(p_credential_id);
  perform public.store_pro_lifecycle_receipt('credential', p_credential_id,
    v_removal.operation_id, v_removal.payload_hash, v_result);
  update public.pro_credential_evidence_removals
  set status = 'complete', storage_path = null, sanitized_result = v_result,
      completed_at = pg_catalog.now(), recovery_operation_id = null
  where id = v_removal.id;
  return v_result;
end;
$$;
...
begin
  ...
  where status = 'complete' and completed_at < pg_catalog.now() - interval '30 days';
...
drop function public.recover_pro_credential_evidence_removal(uuid, uuid);
...
alter function public.prepare_pro_credential_evidence_removal(uuid, uuid, uuid, bigint, uuid, text) owner to postgres;
alter function public.finalize_pro_credential_evidence_removal(uuid, uuid, uuid, bigint, uuid, text) owner to postgres;
alter function public.claim_pro_credential_evidence_removal_recovery(uuid, uuid, uuid, uuid, uuid) owner to postgres;
alter function public.finalize_pro_credential_evidence_removal_recovery(uuid, uuid, uuid, uuid, uuid) owner to postgres;
...
revoke all on table public.pro_credential_evidence_removals
  from public, anon, authenticated, service_role;
revoke all on function public.claim_pro_credential_evidence_removal_recovery(uuid, uuid, uuid, uuid, uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.finalize_pro_credential_evidence_removal_recovery(uuid, uuid, uuid, uuid, uuid)
  from public, anon, authenticated, service_role;
grant execute on function public.claim_pro_credential_evidence_removal_recovery(uuid, uuid, uuid, uuid, uuid)
  to service_role;
grant execute on function public.finalize_pro_credential_evidence_removal_recovery(uuid, uuid, uuid, uuid, uuid)
  to service_role;
