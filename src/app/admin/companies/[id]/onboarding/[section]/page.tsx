...
import { ReopenCompanyOnboardingForm } from '@/components/admin/ReopenCompanyOnboardingForm';
...
import { createAdminOnboardingActionState } from '../action-logic';
import {
  activateAdminCompanyOnboardingAction,
  clearAdminBankIdentifierAction,
  saveAdminActivitiesSectionAction,
  saveAdminBankSectionAction,
  saveAdminEstablishmentSectionAction,
  saveAdminLegalSectionAction,
  saveAdminOfficeSectionAction,
  saveAdminShareholdersSectionAction,
  submitAdminCompanyOnboardingAction,
} from '../actions';
import {
  buildAdminCompanyOnboardingSteps,
  adminCompanyOnboardingSectionHref,
  parseAdminCompanyOnboardingSection,
  type AdminCompanyOnboardingSection,
} from '../route-logic';
...
export default async function AdminCompanyOnboardingSectionPage({
  params,
}: {
  params: Promise<{ id: string; section: string }>;
}) {
  const { id, section: rawSection } = await params;
  const section = parseAdminCompanyOnboardingSection(rawSection);
  if (!section) notFound();
  ...
  const initialState = createAdminOnboardingActionState(snapshot.onboardingVersion);
  const clearBankInitialState = createAdminOnboardingActionState(snapshot.onboardingVersion);
  const reopenInitialState = createAdminOnboardingActionState(snapshot.onboardingVersion);
  const commonLabels = {
    saved: tx('form.saved'),
  ...
  ) as Record<AdminCompanyOnboardingSection, string>;
  const steps = buildAdminCompanyOnboardingSteps(company.id, snapshot, section, sectionLabels);
  const sectionHrefs = {
    ...Object.fromEntries(
      COMPANY_ONBOARDING_SECTION_KEYS.map((key) => [
        key,
        adminCompanyOnboardingSectionHref(company.id, key),
      ]),
    ),
    assignment: `/admin/companies/${encodeURIComponent(company.id)}`,
    workspace: `/admin/companies/${encodeURIComponent(company.id)}`,
  } as Record<CompanyReadinessSection, string>;

  const legalAction = saveAdminLegalSectionAction.bind(null, snapshot.companyId);
  const shareholdersAction = saveAdminShareholdersSectionAction.bind(null, snapshot.companyId);
  const activitiesAction = saveAdminActivitiesSectionAction.bind(null, snapshot.companyId);
  const officeAction = saveAdminOfficeSectionAction.bind(null, snapshot.companyId);
  const establishmentAction = saveAdminEstablishmentSectionAction.bind(null, snapshot.companyId);
  const bankAction = saveAdminBankSectionAction.bind(null, snapshot.companyId);
  const clearBankAction = clearAdminBankIdentifierAction.bind(null, snapshot.companyId);
  const submitAction = submitAdminCompanyOnboardingAction.bind(null, snapshot.companyId);
  const activateAction = activateAdminCompanyOnboardingAction.bind(null, snapshot.companyId);
