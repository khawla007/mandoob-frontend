...
function cmsPage(overrides: Partial<CmsPage> = {}): CmsPage {
  return {
    ...
    canonicalUrl: 'https://mandoob.ae/hello',
    noindex: true,
    schemaMarkup: null,
    createdBy: null,
    updatedBy: null,
...
test('builds published metadata with fallbacks, canonical and noindex robots', () => {
  assert.deepEqual(buildCmsPageMetadata(cmsPage()), {
    ...
    alternates: { canonical: 'https://mandoob.ae/hello' },
    robots: { index: false, follow: false },
  });
  assert.deepEqual(
    buildCmsPageMetadata(
      cmsPage({ metaTitle: null, metaDescription: null, canonicalUrl: null, noindex: false }),
    ),
    {
      title: 'Fallback title',
      description: 'Body',
