...
import { changeStatusSchema } from '@/lib/validation/admin-user';
import { adminChangeStatus } from '@/lib/data/admin-change-status';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const ok = await consumeRateLimit({
      key: `admin-change-status:${session.id}`,
    ...
    await adminChangeStatus(id, parsed.data, {
    ...
    console.error('admin-change-status unexpected', e);
    return errorResponse('INTERNAL', 'Could not change user status', 500);
  }
}
