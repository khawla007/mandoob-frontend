...
import { mfaResetSchema } from '@/lib/validation/admin-user';
import { adminResetMfa } from '@/lib/data/admin-reset-mfa';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const ok = await consumeRateLimit({
      key: `admin-reset-mfa:${session.id}`,
      ...RATE_LIMITS.authedPerUser,
    });
    if (!ok) return errorResponse('RATE_LIMITED', 'Too many requests. Slow down.', 429);

    const raw = await parseJson<unknown>(request);
    const parsed = mfaResetSchema.safeParse(raw);
    if (!parsed.success) {
      return errorResponse('VALIDATION_FAILED', 'Invalid request', 400, {
        issues: parsed.error.issues,
      } as Record<string, unknown>);
    }
    const result = await adminResetMfa(id, parsed.data, {
      caller: { id: session.id, role: session.role!, tenantId: session.tenantId },
      ip: getClientIp(request),
      userAgent: getUserAgent(request),
    });
    return jsonOk({ ok: true, factors_removed: result.factorsRemoved });
  } catch (e) {
    if (e instanceof ApiError) return e.toResponse();
    console.error('admin-reset-mfa unexpected', e);
    return errorResponse('INTERNAL', 'Could not reset MFA', 500);
  }
}
