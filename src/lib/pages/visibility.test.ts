...
test('only already-published, non-deleted pages are public', () => {
  assert.equal(
    isCmsPagePublic({ status: 'published', publishedAt: '2026-07-10T11:59:59.000Z' }, now),
    true,
  );
  assert.equal(isCmsPagePublic({ status: 'draft', publishedAt: now }, now), false);
  assert.equal(
    isCmsPagePublic({ status: 'published', publishedAt: '2026-07-10T12:00:01.000Z' }, now),
    false,
  );
  assert.equal(
    isCmsPagePublic({ status: 'published', publishedAt: now, deletedAt: now }, now),
    false,
  );
