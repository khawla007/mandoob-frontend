...
test('hero opacity, colors, alignments, and safe CSS lengths are validated', () => {
  const valid = pageHeroSettingsSchema.safeParse({
    ...
    buttonAlignment: 'center',
    minHeight: '24rem',
    padding: '1rem 2px 3% auto',
  });
  assert.equal(valid.success, true);
  for (const value of [-0.1, 1.1])
    assert.equal(pageHeroSettingsSchema.safeParse({ overlayOpacity: value }).success, false);
  for (const backgroundColor of ['red', '#12', '#abcd'])
    assert.equal(pageHeroSettingsSchema.safeParse({ backgroundColor }).success, false);
  for (const minHeight of ['calc(100% - 2px)', '10; color:red', '1 2 3 4 5px'])
    assert.equal(pageHeroSettingsSchema.safeParse({ minHeight }).success, false);
...
test('URL fields reject non-HTTP schemes and malformed absolute URLs', () => {
  ...
  assert.equal(
    pageInputSchema.safeParse({ ...base, canonicalUrl: 'https://example.com/about' }).success,
    true,
  );
});

test('button href rejects path confusion, encoded bypasses, and controls', () => {
  for (const buttonHref of [
    '/%5cevil.com',
    '/%2fevil.com',
    '/%252fevil.com',
    '/%255cevil.com',
...
test('page metadata has limits and schema markup must be an object', () => {
  ...
  assert.equal(
    pageInputSchema.safeParse({ ...base, metaDescription: 'x'.repeat(171) }).success,
    false,
  );
  assert.equal(
    pageInputSchema.safeParse({ ...base, schemaMarkup: ['not', 'object'] }).success,
    false,
  );
  assert.equal(
    pageInputSchema.safeParse({ ...base, schemaMarkup: { '@type': 'WebPage' } }).success,
    true,
  );
});
...
test('page script slots are nullable and bounded', () => {
  ...
  for (const key of ['scriptHead', 'scriptBodyStart', 'scriptBodyEnd'] as const) {
    assert.equal(
      pageInputSchema.safeParse({ ...base, [key]: 'x'.repeat(100_001) }).success,
      false,
      key,
    );
  }
