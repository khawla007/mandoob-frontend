...
export function createOpenAiMeetingSummaryProvider(
  opts: {
  ...
  const transcriptionModel =
    opts.transcriptionModel ?? env.OPENAI_TRANSCRIPTION_MODEL ?? DEFAULT_TRANSCRIPTION_MODEL;
  const summaryModel = opts.summaryModel ?? env.OPENAI_SUMMARY_MODEL ?? DEFAULT_SUMMARY_MODEL;
  const maxRecordingBytes =
    opts.maxRecordingBytes ?? parseMaxRecordingBytes(env.MEETING_AI_MAX_RECORDING_BYTES);

  return {
    async processRecording(input) {
      const apiKey = process.env.OPENAI_API_KEY?.trim() || env.OPENAI_API_KEY?.trim();
      if (!apiKey) {
        throw new MeetingAiProviderError(
          'PROVIDER_NOT_CONFIGURED',
          'OpenAI is not configured for meeting summaries.',
        );
      }
      if (!input.recordingUrl) {
        throw new MeetingAiProviderError(
          'RECORDING_NOT_FOUND',
          'Recording is not available for transcription.',
        );
      }
      ...
      if (bytes.byteLength > maxRecordingBytes) {
        throw new MeetingAiProviderError(
          'RECORDING_TOO_LARGE',
          'Recording exceeds the configured AI transcription limit.',
        );
      }
      ...
      form.append(
        'file',
        new Blob([bytes], { type: recording.headers.get('content-type') ?? 'video/mp4' }),
        'recording.mp4',
      );
      ...
      if (typeof transcription.text !== 'string' || !transcription.text.trim()) {
        throw new MeetingAiProviderError(
          ...
          'Transcription response was empty.',
        );
      }
      ...
      if (!validated.ok) {
        throw new MeetingAiProviderError(
          ...
          'Summary response did not match schema.',
        );
      }
...
async function signedRecordingUrl(job: PendingMeetingAiSummary): Promise<string | null> {
  ...
  const { data, error } = await createSupabaseServiceRoleClient()
    .storage.from('tenant-meetings')
    .createSignedUrl(job.recordingStoragePath, 300);
  if (error) return null;
  return data.signedUrl;
}

export async function processPendingMeetingAiSummaries(
  opts: {
    batchSize?: number;
