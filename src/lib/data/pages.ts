...
const cmsPageRowSchema = z.object({
  ...
  hero_settings: heroSettingsRowSchema,
  background_image_media_id: nullableString,
  ...
  noindex: z.boolean(),
  schema_markup: jsonObject.nullable(),
  created_by: nullableString,
  updated_by: nullableString,
  script_head: nullableString,
  script_body_start: nullableString,
  script_body_end: nullableString,
...
export type CmsPageUpsertInput = PageInput & {
  id?: string;
  backgroundImageMediaId?: string | null;
};
...
export function mapCmsPageRow(value: unknown): CmsPage {
  ...
  return {
    ...
    heroSettings: row.hero_settings,
    backgroundImageMediaId: row.background_image_media_id,
    status: row.status,
    ...
    scriptBodyStart: row.script_body_start,
    scriptBodyEnd: row.script_body_end,
...
export async function listAdminCmsPages(
  ...
  const pageSize = Math.min(
    100,
    Math.max(1, Math.trunc(Number.isFinite(options.pageSize) ? options.pageSize! : 8)),
  );
  const from = (page - 1) * pageSize;
  const db = await getSupabase(deps);
  const { data, error, count } = await db
    .from('cms_pages')
    .select(CMS_PAGE_LIST_COLUMNS, { count: 'exact' })
    .is('deleted_at', null)
...
async function canonicalHero(
  db: SupabaseLike,
  heroSettings: PageHeroSettings | undefined,
...
export async function upsertCmsPage(
  ...
  const payload = {
    ...
    hero_settings: canonical.heroSettings,
    background_image_media_id: canonical.mediaId,
    ...
    updated_by: actor.id,
    script_head: parsed.scriptHead ?? null,
    script_body_start: parsed.scriptBodyStart ?? null,
...
export async function softDeleteCmsPage(
  id: string,
  actor: CmsPageActor,
  deps: Deps = {},
): Promise<void> {
  ...
  const { data, error } = await db
    .from('cms_pages')
    .update({ deleted_at: (deps.now ?? new Date()).toISOString(), updated_by: actor.id })
    .eq('id', id)
    .is('deleted_at', null)
    .select('id')
    .single();
...
export async function getPublishedCmsPageBySlug(
  ...
  const { data, error } = await db
    .from('cms_pages')
    .select(CMS_PAGE_PUBLIC_COLUMNS)
    .eq('slug', slug)
    .eq('status', 'published')
    .is('deleted_at', null)
...
export async function listPublishedCmsPages(
  deps: Deps = {},
  ...
  return ((data ?? []) as unknown[]).flatMap((value) => {
    ...
    try {
      ...
      return [
        {
          slug: assertPageSlugAvailable(parsed.data.slug),
