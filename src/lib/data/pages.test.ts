...
const pageRow = (overrides: Row = {}): Row => ({
  ...
  schema_markup: null,
  script_head: null,
  script_body_start: null,
  script_body_end: null,
  created_by: 'creator',
  updated_by: 'updater',
  deleted_at: null,
...
function stub(
  ...
  options: { duplicate?: boolean; queryError?: { message: string; code?: string } } = {},
) {
  ...
  const getRows = (table: string) =>
    tables.get(table) ?? (tables.set(table, []), tables.get(table)!);
  function from(table: string) {
    ...
    const builder = {
      select(...args: unknown[]) {
        selectedColumns = args[0] as string | undefined;
    ...
    function apply(): {
      ...
      if (options.duplicate && (op === 'insert' || op === 'update'))
        return { data: [], error: { message: 'duplicate key', code: '23505' }, count: 0 };
      if (options.queryError) return { data: [], error: options.queryError, count: 0 };
      let rows = getRows(table).filter((r) => filters.every((f) => f(r)));
      if (op === 'insert') {
        const row = pageRow({ id: 'page-new', ...payload });
...
test('stub query builders preserve chains and implement PromiseLike results', async () => {
  const db = stub({
    cms_pages: [pageRow({ status: 'published', published_at: '2026-01-01T00:00:00.000Z' })],
  });
  ...
  assert.equal((await db.from('cms_pages').eq('id', 'page-1').maybeSingle()).data?.id, 'page-1');
});
...
test('query failures use fixed public messages without leaking database details', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      ...
      error.message === 'Unable to list CMS pages' &&
...
test('admin list clamps page and uses exact count, range, and deterministic ordering', async () => {
  const db = stub({
    cms_pages: Array.from({ length: 10 }, (_, i) =>
      pageRow({
        id: `p-${i}`,
        updated_at: `2026-07-${String(i + 1).padStart(2, '0')}T00:00:00.000Z`,
      }),
    ),
  });
  ...
  assert.deepEqual(
    ...
    { total: 10, page: 1, pageSize: 3, size: 3 },
  );
  assert.ok(
    db.calls.some(
      (c) => c.method === 'select' && (c.args[1] as { count?: string })?.count === 'exact',
    ),
  );
  assert.deepEqual(
    db.calls.filter((c) => c.method === 'order').map((c) => c.args[0]),
    ['updated_at', 'id'],
  );
...
test('admin lookup returns drafts and excludes soft deleted rows', async () => {
  ...
  assert.equal(
    db.calls.some((c) => c.method === 'eq' && c.args[0] === 'status'),
    false,
  );
...
test('upsert sanitizes HTML, records actor IDs, and canonicalizes media URL', async () => {
  const db = stub({
    blog_media: [
      { id: 'media-1', public_url: 'https://cdn.example/canonical.jpg', deleted_at: null },
    ],
  });
  const result = await upsertCmsPage(
    {
      ...
      slug: 'page',
      contentJson: {},
      contentHtml: '<p onclick="x">Hi</p><script>x</script>',
      heroSettings: { backgroundImageUrl: 'https://evil.example/x' },
  ...
  assert.equal(insert.updated_by, 'admin-1');
  assert.ok(
    db.calls.some(
      (c) =>
        c.table === 'blog_media' &&
...
test('upsert persists and round-trips all script slots without executing them', async () => {
  ...
  assert.deepEqual(
    {
      script_head: insert.script_head,
      script_body_start: insert.script_body_start,
      script_body_end: insert.script_body_end,
    },
    {
      script_head: scripts.scriptHead,
      script_body_start: scripts.scriptBodyStart,
      script_body_end: scripts.scriptBodyEnd,
    },
  );
  assert.deepEqual(
    {
      scriptHead: result.scriptHead,
      scriptBodyStart: result.scriptBodyStart,
      scriptBodyEnd: result.scriptBodyEnd,
    },
    scripts,
  );
});
...
test('upsert update sets updated actor and clears unbacked client image URL', async () => {
  ...
  assert.equal(update.updated_by, 'admin-2');
  assert.equal(update.created_by, undefined);
});
      ...
      upsertCmsPage(
        {
          ...
          contentJson: {},
          contentHtml: '',
          backgroundImageMediaId: 'missing',
          status: 'draft',
        },
        { id: 'a', role: 'admin' },
        { supabase: missing },
      ),
    (e: unknown) => e instanceof ApiError && e.code === 'INVALID_MEDIA',
  );
  const duplicate = stub({}, { duplicate: true });
  await assert.rejects(
    () =>
      upsertCmsPage(
        { title: 'P', slug: 'p', contentJson: {}, contentHtml: '', status: 'draft' },
        { id: 'a', role: 'admin' },
        { supabase: duplicate },
      ),
    (e: unknown) => e instanceof ApiError && e.code === 'DUPLICATE_SLUG',
  );
});
...
test('upsert rejects deleted background media without persisting the page', async () => {
  const db = stub({
    blog_media: [
      {
        id: 'media-deleted',
        public_url: 'https://cdn.example/deleted.jpg',
      ...
      upsertCmsPage(
        {
          ...
          contentJson: {},
          contentHtml: '',
          backgroundImageMediaId: 'media-deleted',
          status: 'draft',
        },
        { id: 'a', role: 'admin' },
        { supabase: db },
  ...
  assert.equal(
    db.calls.some((c) => c.table === 'cms_pages' && c.method === 'insert'),
    false,
  );
});
...
test('soft delete records actor and timestamp', async () => {
  ...
  const update = db.calls.find((c) => c.method === 'update')?.args[0] as Row;
  assert.equal(update.updated_by, 'admin-3');
  assert.equal(typeof update.deleted_at, 'string');
});

test('update maps missing and already-deleted page IDs to NOT_FOUND', async () => {
  for (const db of [
    stub(),
    stub({ cms_pages: [pageRow({ deleted_at: '2026-07-01T00:00:00.000Z' })] }),
  ]) {
    await assert.rejects(
      () =>
        upsertCmsPage(
...
test('public lookup applies published, date, and deletion filters', async () => {
  ...
  assert.equal(
    (
      await getPublishedCmsPageBySlug('hello', {
        supabase: db,
  ...
  assert.ok(
    db.calls.some((c) => c.method === 'eq' && c.args[0] === 'status' && c.args[1] === 'published'),
  );
...
test('published sitemap list returns only public columns in deterministic order', async () => {
  ...
  assert.deepEqual(pages, [
    { slug: 'hello', updatedAt: '2026-07-02T00:00:00.000Z', noindex: false },
  ]);
  assert.ok(
...
test('published sitemap list rejects malformed projection rows', async () => {
  for (const bad of [{ updated_at: null }, { noindex: 'false' }]) {
    const db = stub({
      cms_pages: [
        pageRow({ status: 'published', published_at: '2026-01-01T00:00:00.000Z', ...bad }),
      ],
    });
    await assert.rejects(
