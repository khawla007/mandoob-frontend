...
export function PageHeroSettings({
  state,
  onChange,
}: {
  ...
  return (
    <fieldset className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <F label="Text" id="hero-text">
          <Textarea
            ...
            maxLength={1000}
            onChange={(e) => field('text', e.currentTarget.value || null)}
          />
        </F>
      </div>
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <F label="Button URL" id="hero-button-href">
          <Input
            ...
            onChange={(e) => field('buttonHref', e.currentTarget.value || null)}
          />
        </F>
      </div>
        ...
        {(['headingAlignment', 'textAlignment', 'buttonAlignment'] as const).map((key) => (
          <F key={key} label={key.replace('Alignment', ' alignment')} id={`hero-${key}`}>
            <select
              ...
              onChange={(e) => field(key, e.currentTarget.value as (typeof alignments)[number])}
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <F label="Overlay color" id="hero-overlay">
          <Input
            id="hero-overlay"
            type="color"
            value={state.settings.overlayColor}
        ...
        <F label="Overlay opacity" id="hero-opacity">
          <Input
            ...
            onChange={(e) => field('overlayOpacity', Number(e.currentTarget.value))}
          />
        </F>
      </div>
        ...
        {(['minHeight', 'maxWidth', 'padding', 'margin'] as const).map((key) => (
          <F key={key} label={key.replace(/([A-Z])/g, ' $1')} id={`hero-${key}`}>
            <Input
              id={`hero-${key}`}
              value={state.settings[key] ?? ''}
              placeholder={key === 'padding' || key === 'margin' ? '0 1rem' : 'auto'}
      ...
      <PageHeroMediaPicker
        media={{ id: state.mediaId, previewUrl: state.previewUrl }}
        onChange={(media) =>
          onChange({
            ...state,
            mediaId: media.id,
            previewUrl: media.previewUrl,
            settings: { ...state.settings, backgroundImageUrl: media.previewUrl },
          })
        }
      />
    </fieldset>
