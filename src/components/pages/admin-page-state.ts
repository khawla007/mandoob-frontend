...
export function createHeroState(
  settings: Partial<PageHeroSettings> = {},
  ...
  return {
    settings: {
      ...DEFAULT_HERO,
      ...settings,
      backgroundImageUrl: previewUrl ?? settings.backgroundImageUrl ?? null,
    },
    mediaId,
    previewUrl: previewUrl ?? settings.backgroundImageUrl ?? null,
  };
}

export function updateHeroField<K extends keyof PageHeroSettings>(
  state: HeroEditorState,
  key: K,
  value: PageHeroSettings[K],
): HeroEditorState {
...
export function serializeHeroState(state: HeroEditorState): string {
  const settings: Record<string, unknown> = {
    ...state.settings,
    backgroundImageMediaId: state.mediaId,
  };
...
export function pageHref(page: number): string {
  return page <= 1 ? '/admin/pages' : `/admin/pages?page=${page}`;
}
