...
import { nextDialogFocusIndex } from './admin-page-state';
  ...
  useEffect(() => {
    ...
    const keyboard = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Tab') {
        ...
        controls[nextDialogFocusIndex(current < 0 ? (event.shiftKey ? 0 : -1) : current, controls.length, event.shiftKey)]?.focus();
      }
    };
